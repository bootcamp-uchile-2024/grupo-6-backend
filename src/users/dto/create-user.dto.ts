...
export class CreateUserDto {
    @ApiProperty({example: 'Roberto Andres', description: 'Primer y segundo nombre del usuario (debe contener un formato de string correcto).'})
    @IsString({message: "Introducir un formato de nombre correcto."})
    public nombres: string;
    @ApiProperty({example: 'Gonzalez', description: 'Apellido paterno del usuario (debe contener un formato de string correcto).'})
    @IsString({message: "Introducir un formato de nombre correcto."})
    public apellidoPaterno: string;
    @ApiProperty({example: 'Ramirez', description: 'Apellido materno del usuario (debe contener un formato de string correcto).'})
    @IsString({message: "Introducir un formato de nombre correcto."})
    public apellidoMaterno: string;
    @ApiProperty({example: 'r.gonz.ram@gmail.com', description: 'Correo electronico del usuario (debe ser un correo con formato válido).'})
    ...
    @ApiProperty({example: ClientState.ACTIVO, description: 'Estado del usuario, puede ser Activo o Baneado.', enum: ClientState})
    @IsEnum(ClientState,{message: "El estado del usuario tiene que ser el formato correcto. Ej: 'Activo' o 'Baneado'."})
