...
private usuarios: User[] = [
  new User(1,"Cristobal Andres", "Sanchez","Ossandon","c.sanch.oss@gmail.com","qwerty",[new Address(1,"El condor","125","N/A","Nunoa","Santiago","Region Metropolitana",[TipoDireccion.ENVIO,TipoDireccion.FACTURACION],"CASA")],ClientType.PREMIUM,ClientState.ACTIVO),
  new User(2,"Daniel Antonio", "Fernandez","Correa","d.fernandez.correa@gmail.com","12345",[new Address(1,"San Marcos","27","N/A","Penalolen","Santiago","Region Metropolitana",[TipoDireccion.ENVIO,TipoDireccion.FACTURACION],"CASA")],ClientType.ESTANDAR,ClientState.ACTIVO)
];
...
createSHA256Hash(inputString) {
  ...
  createAddress(idUsuario: number, 
    calle: string,
    numeroCalle: string,
    comuna: string,
    ciudad: string,
    ...
    informacionAdicional?: string): Address {
    const elementoUs: number = this.usuarios.findIndex((element: User) => element.idUsuario == idUsuario)
    if(elementoUs == - 1){
      throw new HttpException('No existe usuario con el id ingresado.', 404);    }
    
    let newAdress: Address = new Address(0,
      '',
      '',
      '',
      '',
  ...
  updateAddress(idUsuario: number, 
    idDireccion: number, 
    calle?: string,
    numeroCalle?: string,
    numeroDepartamento?: string,
    ...
    if(elementoUs == - 1){
      throw new HttpException('No existe usuario con el id ingresado.', 404);    }
    const elementoAd: number = this.usuarios[elementoUs].direccion.findIndex((element: Address) => element.idDireccion == idDireccion)
    if(elementoAd == - 1){
    ...
    if(numeroDepartamento){
      this.usuarios[elementoUs].direccion[elementoAd].numeroDepartamento = numeroDepartamento
    }
    if(comuna){
    ...
    if(tipoDireccion){
      if(!isArray(tipoDireccion)){
        ...
        this.usuarios[elementoUs].direccion[elementoAd].tipoDireccion.push(tipoDireccion as TipoDireccion);
      }
      else{
        this.usuarios[elementoUs].direccion[elementoAd].tipoDireccion = [];
        this.usuarios[elementoUs].direccion[elementoAd].tipoDireccion = tipoDireccion as TipoDireccion[];
      }
    }
    if(informacionAdicional){
      this.usuarios[elementoUs].direccion[elementoAd].informacionAdicional = informacionAdicional
    }
  ...
  remove(id: number) {
    const elemento: number = 
      this.usuarios.findIndex((element: User) => element.idUsuario == id)
    if(elemento == -1){
      throw new Error();
    }
  ...
  removeAddress(idUser: number,
    idAdress: number
  ) {
    const elementoUs: number = this.usuarios.findIndex((element: User) => element.idUsuario == idUser)
    if(elementoUs == - 1){
      throw new HttpException('No existe usuario con el id ingresado.', 404)    }
    const elementoAd: number = this.usuarios[elementoUs].direccion.findIndex((element: Address) => element.idDireccion == idAdress)
    if(elementoAd == - 1){
      throw new HttpException('No existe direccion con el id ingresado.', 404)    }
